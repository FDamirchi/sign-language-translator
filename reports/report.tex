...
\newcommand{\StudentOneID}{شماره دانشجویی اول}
...
\begin{warningbox}[نکته مهم پیش از تحویل]
کد آموزش میانگین \eng{loss} را محاسبه می‌کند و زمان‌بند نرخ یادگیری را بر مبنای \eng{validation loss} به‌روزرسانی می‌کند، اما مقادیر تاریخی \eng{train loss} و \eng{validation loss} در خروجی نوت‌بوک ذخیره یا رسم نشده‌اند. همچنین وجود شکل نهایی ماتریس درهم‌ریختگی در خروجی متعهدشده نوت‌بوک قابل تأیید نیست. چون صورت پروژه این دو خروجی را صریحاً الزامی کرده است، پیش از تحویل باید نوت‌بوک یک‌بار با ذخیره تاریخچه زیان و رسم ماتریس درهم‌ریختگی اجرا شود. در این گزارش هیچ مقدار زیان تخمین زده نشده است.
\end{warningbox}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{assets/class_distribution.png}
\caption{توزیع کلاس‌ها در پوشه آموزشی و آزمون اولیه؛ خروجی مستقیم نوت‌بوک فاز ۱}
...
عرض & حداقل، حداکثر و میانگین همگی \eng{200} پیکسل \\
ارتفاع & حداقل، حداکثر و میانگین همگی \eng{200} پیکسل \\
...
\end{align*}
پس از تبدیل پیکسل‌ها به بازه \eng{[0,1]}، هر کانال طبق رابطه زیر استاندارد شد:
\[
 x'_{c}=\frac{x_c-\mu_c}{\sigma_c}
\]
آمار محاسبه‌شده در فایل \code{dataset_stats.pt} ذخیره و عیناً در تنظیمات استنتاج بلادرنگ استفاده شده است. این همسانی برای جلوگیری از تغییر توزیع ورودی میان آموزش و اجرا ضروری است.
...
بخش عمده پارامترهای مدل در لایه \eng{FC1} قرار دارد. این انتخاب ظرفیت طبقه‌بندی بالایی ایجاد می‌کند، اما هزینه حافظه و ریسک بیش‌برازش را نیز افزایش می‌دهد. وجود \eng{Dropout} و افزایش داده برای کنترل این ریسک اهمیت دارد.

\section{افزایش داده و کنترل بیش‌برازش}
تبدیلات مسیر آموزش عبارت‌اند از:
\begin{itemize}
  \item تغییر اندازه به \eng{200×200}؛
  \item چرخش تصادفی تا \lr{$\pm 10^\circ$}؛
  \item وارونگی افقی تصادفی؛
  \item تغییر جزئی روشنایی و کنتراست با شدت \eng{0.1}؛
  \item تبدیل به تنسور و نرمال‌سازی با آمار داده آموزش.
\end{itemize}
...
\begin{successbox}[تکنیک‌های استفاده‌شده برای بهبود تعمیم]
در معماری نهایی دو مورد از پیشنهادهای بخش امتیازی صورت پروژه پیاده‌سازی شده است: \eng{Data Augmentation} و \eng{Dropout}. همچنین \eng{ReduceLROnPlateau} برای کاهش نرخ یادگیری در صورت توقف بهبود زیان اعتبارسنجی به کار رفته است. \eng{Batch Normalization} و اتصالات میانبر در نسخه فعلی استفاده نشده‌اند.
\end{successbox}
...
\caption{دقت آموزش و اعتبارسنجی؛ بازسازی‌شده از خروجی‌های متنی نوت‌بوک}
\label{fig:acc}
\end{figure}
...
بالاتربودن دقت اعتبارسنجی نسبت به دقت آموزش الزاماً نشانه خطا نیست. در زمان آموزش، تبدیلات تصادفی و \eng{Dropout} مسئله را سخت‌تر می‌کنند؛ درحالی‌که هنگام اعتبارسنجی شبکه در حالت \eng{eval} قرار دارد و ورودی‌ها بدون افزایش داده‌اند. بهترین مدل در دوره نهم ذخیره شده است.
...
بهترین دقت اعتبارسنجی & \eng{99.23\%} \\
\bottomrule
\end{tabularx}
\end{table}

\begin{infobox}[تفسیر نتیجه]
