...
\section{بهبود مدل با Weighted Loss (Fine-Tuning)}
...
\subsection{مرحله ۱: وزن‌دهی به کلاس‌های A، M و N}

کلاس‌های A، M و N از جمله کلاس‌هایی بودند که در تشخیص Real-time بیشترین خطا را داشتند. هر سه این حروف در زبان اشاره به‌صورت مشت با تغییرات جزئی در موقعیت شست و انگشتان اجرا می‌شوند که تشخیص آنها را دشوار می‌کند. به همین دلیل، وزن این کلاس‌ها در تابع هزینه به مقدار \eng{2.5} افزایش یافت. سپس مدل با این وزن‌های جدید به مدت \eng{5} دوره (Epoch) با نرخ یادگیری \eng{0.0001} Fine-tune شد. پس از این مرحله، دقت مدل روی این سه کلاس به‌طور محسوسی بهبود یافت و خطای تشخیص آنها کاهش پیدا کرد.

\subsection{مرحله ۲: وزن‌دهی به کلاس‌های A، S و T}

در مرحله بعد، کلاس‌های A، S و T مورد بررسی قرار گرفتند. این کلاس‌ها نیز به دلیل شباهت در حالت مشت، در محیط Real-time با چالش مواجه بودند. به‌ویژه کلاس S که تشخیص آن در شرایط نوری مختلف با دشواری همراه بود. بنابراین، وزن این کلاس‌ها نیز به مقدار \eng{2.5} تنظیم شد. این مرحله با \eng{3} دوره Fine-tune انجام شد تا مدل تفاوت‌های ظریف بین این کلاس‌ها را بهتر تشخیص دهد. نتایج نشان داد که دقت تشخیص این کلاس‌ها به‌ویژه S و T در تصاویر Real-time بهبود یافته است.

\subsection{مرحله ۳: یکسان‌سازی وزن‌ها}

پس از بهبود کلاس‌های هدف، برای جلوگیری از Overfitting و حفظ تعادل مدل، وزن‌های همه کلاس‌ها به مقدار \eng{1} برگردانده شد. این کار باعث شد مدل بدون اولویت‌دهی خاص، روی همه کلاس‌ها به‌صورت یکسان تمرکز کند و عملکرد کلی آن پایدار بماند. در نهایت، مدل با وزن‌های یکسان به مدت \eng{3} دوره دیگر Fine-tune شد تا وزن‌های نهایی تثبیت شوند و مدل برای استفاده در محیط Real-time آماده گردد.

\subsection{نتایج حاصل از Fine-Tuning}
