...
\settextfont[
  BoldFont={B Nazanin},
  ItalicFont={B Nazanin},
  BoldItalicFont={B Nazanin},
  Scale=1.16
]{B Nazanin}
...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \node[fill=white,draw=white,minimum size=2.35cm,circle,inner sep=4mm]
    ...
    {\includegraphics[width=1.65cm]{\UniversityLogo}};
...
\begin{infobox}[دامنه سامانه]
سامانه حاضر یک مترجم حروف‌به‌حروف مبتنی بر \eng{fingerspelling} است. بنابراین خروجی آن از توالی کلاس‌های ایستا ساخته می‌شود و جایگزین یک مدل کامل ترجمه زبان اشاره طبیعیِ پیوسته، شامل حرکات پویا و دستور زبان مستقل زبان اشاره، نیست.
\end{infobox}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{assets/train_distribution.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{assets/sample_grid.png}
...
عرض & میانگین همگی \eng{200} پیکسل \\
ارتفاع & میانگین همگی \eng{200} پیکسل \\
...
همان‌طور که در جدول بالا مشاهده می‌شود، تمام تصاویر موجود در مجموعه داده از ابعاد یکسان و حالت رنگی \eng{RGB} برخوردار هستند که این امر فرآیند پیش‌پردازش را به‌طور قابل‌توجهی ساده می‌سازد. همچنین داده‌ها از توازن کامل برخوردار بوده و هر یک از ۲۹ کلاس دقیقاً شامل ۳\,۰۰۰ تصویر می‌باشند. این توازن کامل در کنار یکنواختی ابعاد تصاویر، مجموعه داده را به گزینه‌ای ایده‌آل برای آموزش مدل‌های یادگیری عمیق تبدیل کرده است و نیاز به تکنیک‌های متعادل‌سازی یا تغییر اندازه را به‌حداقل می‌رساند.
...
\midrule
\eng{Train} & \eng{65,250} & \eng{75\%} \\
\eng{Validation} & \eng{13,050} & \eng{15\%} \\
\eng{Test} & \eng{8,700} & \eng{10\%} \\
\midrule
جمع & \eng{87,000} & \eng{100\%} \\
\bottomrule
\end{tabular}
\end{table}

این تقسیم‌بندی بر اساس رویه‌های استاندارد در پروژه‌های یادگیری ماشین انتخاب شده است. نسبت ۷۵ درصد برای داده‌های آموزش، حجم مناسبی از داده‌ها را برای یادگیری الگوها در اختیار مدل قرار می‌دهد. استفاده از ۱۵ درصد داده‌ها برای اعتبارسنجی، امکان پایش عملکرد مدل در حین آموزش و تنظیم فراپارامترها را بدون دخالت در فرآیند یادگیری فراهم می‌کند. همچنین در نظر گرفتن ۱۰ درصد داده‌ها به‌عنوان مجموعه آزمون، ارزیابی نهایی و بی‌طرفانه از عملکرد مدل روی داده‌های کاملاً دیده‌نشده را امکان‌پذیر می‌سازد. این نسبت‌ها به‌خوبی تعادل بین آموزش کافی، اعتبارسنجی دقیق و ارزیابی قابل‌اطمینان را برقرار می‌کنند.

\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{assets/dataset_split.png}
...
آمار محاسبه‌شده در فایل \code{dataset_stats.pt} ذخیره شده و در فرآیند استنتاج بلادرنگ (Real-time) نیز از همین مقادیر استفاده می‌شود. این همسانی در نرمال‌سازی، از تغییر توزیع داده‌های ورودی میان فاز آموزش و فاز اجرا جلوگیری کرده و یکی از عوامل کلیدی در حفظ دقت مدل در محیط عملیاتی است.
...
تعداد ۴ لایه کانولوشن بر اساس ویژگی‌های داده و آزمایش‌های اولیه انتخاب شده است. تصاویر حروف اشاره شامل الگوهای محدودی هستند و ۴ لایه برای استخراج ویژگی‌های ساده تا پیچیده کافی است. همچنین با توجه به ابعاد ۲۰۰×۲۰۰ پیکسل، پس از ۴ بار بیشینه‌گیری، ابعاد به ۱۲×۱۲ کاهش می‌یابد که حفظ اطلاعات مکانی را تضمین می‌کند. آزمایش‌های اولیه با ۳ و ۵ لایه نشان داد که ۳ لایه دقت کافی ندارد و ۵ لایه نیز بدون بهبود چشمگیر، تعداد پارامترها و خطر بیش‌برازش را افزایش می‌دهد. بنابراین ۴ لایه به‌عنوان معماری نهایی انتخاب شده است.
...
\subsubsection{لایه \eng{FC1} با ۵۱۲ نورون}

۵۱۲ نورون در لایه پنهان تمام‌متصل، ظرفیت کافی برای یادگیری ترکیبات پیچیده ویژگی‌ها را فراهم می‌کند. این تعداد نه‌چندان کم است که قدرت طبقه‌بندی را محدود کند و نه‌چندان زیاد که باعث بیش‌برازش شود. با توجه به تعداد کلاس‌ها (۲۹ کلاس)، این تعداد نورون انتخاب مناسبی است.

\subsubsection{\eng{Dropout} با نرخ ۰.۵}

با توجه به تعداد بالای پارامترها در لایه \eng{FC1} (حدود ۱۹ میلیون)، \eng{Dropout} با نرخ ۰.۵ به‌عنوان یک روش تنظیم‌کننده (Regularization) استفاده شده است. این نرخ به‌خوبی خطر بیش‌برازش را کاهش می‌دهد و در عین حال اجازه می‌دهد مدل اطلاعات مفید را یاد بگیرد.

\subsubsection{لایه خروجی ۲۹ نورون}

تعداد نورون‌های لایه خروجی دقیقاً برابر با تعداد کلاس‌های مسئله (۲۹ کلاس: \eng{A-Z, del, nothing, space}) است. هر نورون نشان‌دهنده احتمال تعلق تصویر ورودی به یکی از کلاس‌ها است و خروجی نهایی با تابع \eng{Softmax} به توزیع احتمال تبدیل می‌شود.


\section{افزایش داده و کنترل بیش‌برازش}

به منظور افزایش قدرت تعمیم‌دهی مدل و جلوگیری از بیش‌برازش، از مجموعه‌ای از تبدیلات تصادفی در مرحله آموزش استفاده شده است. این تبدیلات شامل تغییر اندازه به \eng{200×200}، چرخش تصادفی تا \lr{$\pm 10^\circ$}، وارونگی افقی تصادفی و تغییر جزئی روشنایی و کنتراست با شدت ۰.۱ است. پس از اعمال این تبدیلات، تصاویر به تنسور تبدیل شده و با استفاده از آمار محاسبه‌شده از داده‌های آموزش (میانگین و انحراف معیار هر کانال) نرمال‌سازی می‌شوند. برای داده‌های اعتبارسنجی و آزمون، تنها تغییر اندازه، تبدیل به تنسور و نرمال‌سازی انجام می‌شود تا ارزیابی روی داده‌های بدون اغتشاش صورت گیرد.
...
\begin{enumerate}
    \item \textbf{\eng{Data Augmentation}:} با استفاده از تبدیلات تصادفی، تنوع داده‌های آموزش به‌طور مصنوعی افزایش یافته است.
    \item \textbf{\eng{Dropout}:} لایه \eng{Dropout} با نرخ ۰.۵ پس از لایه \eng{FC1} قرار گرفته است تا وابستگی نورون‌ها به یکدیگر کاهش یابد.
    \item \textbf{\eng{ReduceLROnPlateau}:} نرخ یادگیری در صورت توقف بهبود زیان اعتبارسنجی به‌صورت خودکار کاهش می‌یابد تا مدل از نقاط بهینه محلی عبور کند.
\end{enumerate}

لازم به ذکر است که \eng{Batch Normalization} و اتصالات میانبر (\eng{Residual Connections}) در نسخه فعلی استفاده نشده‌اند، اما در صورت نیاز به بهبود بیشتر عملکرد، می‌توانند به معماری اضافه شوند.
...
\begin{itemize}
    ...
    \item \textbf{مقاومت در برابر نویز:} داده‌های تصویری شامل نویز هستند و Adam با استفاده از میانگین متحرک گرادیان‌ها، این نویز را کاهش می‌دهد.
    \item \textbf{مناسب برای داده‌های بزرگ:} با توجه به حجم بالای داده‌های آموزش (۸۷,۰۰۰ تصویر)، Adam عملکرد پایدار و مناسبی ارائه می‌دهد.
\end{itemize}

اگرچه \eng{SGD} نیز گزینه‌ای رایج است، اما به دلیل همگرایی کندتر و نیاز به تنظیم دقیق‌تر نرخ یادگیری، برای این پروژه با محدودیت تعداد دوره، انتخاب مناسبی نبود. تجربه عملی نیز نشان داده است که Adam در اکثر پروژه‌های بینایی ماشین به نتایج بهتری می‌رسد.

\section{روند آموزش}

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{assets/training_accuracy.png}
...
\midrule
1 & \eng{79.39\%} & \eng{93.24\%} & 6 & \eng{94.14\%} & \eng{98.79\%} \\
2 & \eng{87.02\%} & \eng{96.35\%} & 7 & \eng{94.72\%} & \eng{98.70\%} \\
3 & \eng{90.20\%} & \eng{97.36\%} & 8 & \eng{95.29\%} & \eng{98.57\%} \\
4 & \eng{92.05\%} & \eng{98.01\%} & 9 & \eng{95.79\%} & \eng{99.23\%} \\
5 & \eng{93.38\%} & \eng{98.80\%} & 10 & \eng{95.88\%} & \eng{98.81\%} \\
\bottomrule
\end{tabular}
\end{table}

در ابتدا، فرآیند آموزش با ۳۰ دوره برنامه‌ریزی شده بود. اما پس از مشاهده روند تغییرات دقت و زیان در دوره‌های ابتدایی، مشخص شد که مدل به سرعت به دقت بالایی روی داده‌های اعتبارسنجی می‌رسد و از دوره پنجم به بعد، دقت اعتبارسنجی در محدوده ۹۸ تا ۹۹ درصد تثبیت می‌شود. همچنین از دوره ششم به بعد، بهبود دقت اعتبارسنجی بسیار ناچیز بوده و در برخی دوره‌ها حتی کاهش جزئی نیز مشاهده می‌شود. از سوی دیگر، دقت آموزش همچنان به آرامی افزایش می‌یابد که نشان‌دهنده نزدیک شدن مدل به نقطه بیش‌برازش است.

با توجه به این مشاهدات، تعداد دوره‌ها از ۳۰ به ۱۰ کاهش یافت تا از اتلاف وقت و افزایش ریسک بیش‌برازش جلوگیری شود. این تصمیم با بررسی رفتار مدل در دوره‌های ابتدایی و توقف بهبود چشمگیر دقت اعتبارسنجی، اتخاذ گردید. انتخاب ۱۰ دوره به‌عنوان تعداد نهایی دوره‌ها، تعادل مناسبی بین دستیابی به دقت بالا و جلوگیری از بیش‌برازش ایجاد می‌کند.

همان‌طور که در جدول و نمودار مشاهده می‌شود، بهترین دقت اعتبارسنجی در دوره نهم با مقدار ۹۹.۲۳ درصد ثبت شده است. بنابراین وزن‌های مربوط به این دوره به‌عنوان بهترین مدل ذخیره شده و در ادامه مراحل استنتاج و ارزیابی نهایی مورد استفاده قرار گرفته است. عملکرد مدل در دوره دهم با کاهش جزئی دقت اعتبارسنجی همراه بود که نشان‌دهنده شروع بیش‌برازش است و تأیید می‌کند که توقف در دوره نهم انتخاب مناسبی بوده است.

\section{ارزیابی نهایی}

نتایج ثبت‌شده روی مجموعه آزمون ۸\,۷۰۰ تصویری عبارت‌اند از:
...
\caption{ماتریس درهم‌ریختگی (Confusion Matrix) روی مجموعه آزمون}
\label{fig:confusion}
\end{figure}
...
این خطاها عمدتاً بین کلاس‌هایی رخ داده است که از نظر شکل دست و موقعیت انگشتان شباهت زیادی به یکدیگر دارند. برای مثال، حروف \eng{M} و \eng{N} در زبان اشاره تنها در تعداد انگشتان باز شده تفاوت دارند و این شباهت بصری باعث سردرگمی مدل شده است. همچنین حروف \eng{W} و \eng{V} نیز از نظر زاویه انگشتان بسیار نزدیک به هم هستند. مجموع خطاها ۵۷ مورد از ۸\,۷۰۰ تصویر (معادل ۰.۶۶ درصد) است که نشان‌دهنده عملکرد بسیار دقیق مدل است.
% ==================== Chapter 4 ====================
